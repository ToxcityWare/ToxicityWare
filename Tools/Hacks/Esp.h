...
public:
	...
	static void Watermark()
	{
		...
		if (Options.Visuals.VisualsMisc.bWatermark)
		{
			...
			{
			case 0:change = (""); break;
			case 1:change = ("A"); break;
			case 2:change = ("Au"); break;
			case 3:change = ("Aus"); break;
			case 4:change = ("Ausk"); break;
			case 5:change = ("Ausky"); break;
			case 6:change = ("usky"); break;
			case 7:change = ("sky"); break;
			case 8:change = ("ky"); break;
			case 9:change = ("y"); break;
			...
			Interfaces.pEngine->GetScreenSize(W, H);
			Interfaces.pSurface->DrawT(cW - 955, cH, CColor(138, 42, 226, 255), Hacks.Font_Watermark, false, "> %s", change.c_str());
			...
			if (Options.Visuals.VisualsMisc.bTime)
				Interfaces.pSurface->DrawT(cW - 955, cH + 13, CColor(138, 42, 226, 255), Hacks.Font_Watermark, false, "> Time: %s", GetTimeString().c_str());
	...
	static void DrawInfo(RECT rect, CBaseEntity* pPlayer)
	{
		...
		else if (Interfaces.pEngine->GetPlayerInfo(pPlayer->GetIndex(), &info))
		{
			...
			if (Options.Visuals.Esp.EspMisc.bIsScoped && pPlayer->m_bIsScoped())
				Interfaces.pSurface->DrawT(middle, rect.top - top++ * 9, CColor(138, 42, 226, 255), Hacks.Font_ESP, true, "Aiming");

			if (Options.Visuals.Esp.EspMisc.bIsFlashed && pPlayer->IsFlashed())
				Interfaces.pSurface->DrawT(middle, rect.top + top++ * 9, CColor(138, 42, 226, 255), Hacks.Font_ESP, true, "Flashed");

			if (Options.Visuals.Esp.EspBox.bPitch)
			{
				float Pitch = pPlayer->GetEyeAngles().x;
				Interfaces.pSurface->DrawT(middle, rect.bottom + bot++ * 9, CColor(138, 42, 226, 255), Hacks.Font_ESP, true, "Pitch: %.1f", Pitch);
			}
			if (Options.Visuals.Esp.EspBox.bYaw)
			{
				float Yaw = pPlayer->GetEyeAngles().y;
				Interfaces.pSurface->DrawT(middle, rect.bottom + bot++ * 9, CColor(138, 42, 226, 255), Hacks.Font_ESP, true, "Yaw: %.1f", Yaw);
			}
				...
				if (!pEntity->BombDefused())
				{
					...
					if (flTime > 0.f)
					{
						...
						if (flTime > 5.f)
						{
							bool bCanDefuse = (Hacks.LocalPlayer->HasKit() && flTime > 5.5f) || flTime > 10.5f;
							if (flTime < 10.f)
								Interfaces.pSurface->DrawT(vPos.x + 25, vPos.y, bCanDefuse ? CColor(0, 212, 255, 255) : CColor(0, 212, 255, 255), Hacks.Font_Watermark, false, "[C4]: %.1f (Kit Needed)", flTime); // 2 - blue
						}

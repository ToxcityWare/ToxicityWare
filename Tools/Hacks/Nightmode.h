...
void doColorModulation()
{
	...
	static bool Performed = false, ColorMod;
	...
	*(float*)((DWORD)&sv_skyname->fnChangeCallback + 0xC) = NULL;
	...
	if (!Performed)
	{
		for (auto i = Interfaces.pMaterialSystem->FirstMaterial(); i != Interfaces.pMaterialSystem->InvalidMaterial(); i = Interfaces.pMaterialSystem->NextMaterial(i))
		{
			...
			{
				if (strstr(pMaterial->GetTextureGroupName(), "Model")) {
					pMaterial->ColorModulate(Options.Col.colormodModels[0], Options.Col.colormodModels[1], Options.Col.colormodModels[2]);
				}

				if (strstr(pMaterial->GetTextureGroupName(), "World"))
				{
					sv_skyname->SetValue("sky_csgo_night02");
					pMaterial->ColorModulate(Options.Col.colormod[0], Options.Col.colormod[1], Options.Col.colormod[2]);
				}

				if (strstr(pMaterial->GetTextureGroupName(), "StaticProp"))
				{
					pMaterial->ColorModulate(Options.Col.colormodProps[0], Options.Col.colormodProps[1], Options.Col.colormodProps[2]);
				}
			}
			else
			{
	...
	if (ColorMod != Options.Visuals.VisualsWorlds.bColorMod)
	{
		ColorMod = Options.Visuals.VisualsWorlds.bColorMod;
		Performed = false;
	}
}